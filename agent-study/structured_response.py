...
class CustomerServiceResponse(BaseModel):
    """客服回答的结构化输出格式。"""
    answer: str = Field(description="给用户的回答内容")
    source: Literal["knowledge_base", "tool", "general_knowledge"] = Field(
        description="回答信息的来源：knowledge_base=内部知识库, tool=工具调用, general_knowledge=通用知识"
    )
    confidence: Literal["high", "medium", "low"] = Field(
        description="对回答准确性的置信度"
    )
    needs_human: bool = Field(
        description="是否建议转接人工客服"
    )
...
llm = ChatOpenAI(
    ...
    extra_body={"enable_thinking": False},  # 关闭思考模式，否则不支持 tool_choice=required
)
...
agent = create_agent(
    ...
    response_format=CustomerServiceResponse, # 结构化输出：LLM 返回值会被解析为 Pydantic 模型
...
while True:
    ...
    # 从结果中提取结构化响应（Pydantic 模型实例）
    response: CustomerServiceResponse = result["structured_response"]

    # 展示结构化输出的各字段
    print(f"客服: {response.answer}")
    print(f"  [来源: {response.source} | 置信度: {response.confidence} | 需要人工: {'是' if response.needs_human else '否'}]")
    print(f"{'-' * 50}")